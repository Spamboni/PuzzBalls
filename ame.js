...
class Game {
  ...
  _bindInput() {
    ...
    function onDown(e) {
      ...
      var pos = getPos(e);
      // World position — account for view scroll (canvas translate)
      ...
      // ── Editor mode ──────────────────────────────────────────────────────────
      if (self._editorMode) {
        // Canvas is translated by _viewScrollY during draw, so button rects are in
        // world space. Convert screen tap to world coords by subtracting scroll.
        var _vSY = self._viewScrollY || 0;
        var _py  = pos.y - _vSY;
        var _px  = pos.x;
        var inPanel = _py >= self.floorY();  // world-space check
        ...
        // Scroll — chute strip drag (right 50px, above floor in world space)
        // Also allow scroll from panel area that didn't hit a button
        var chuteX = self.W - 50;
        if (inPanel || (pos.x >= chuteX && _py < self.floorY())) {
      ...
      if (!self._editorMode) {
        ...
        // ── Chute ball buttons — drop balls when tapped ─────────────────────
        ...
        if (self._chuteDeleteRect) {
          var cdr = self._chuteDeleteRect;
          if (pos.x >= cdr.x && pos.x <= cdr.x + cdr.w &&
              pos.y >= cdr.y && pos.y <= cdr.y + cdr.h) {
    ...
    function onMove(e) {
      ...
      if (self._tubeRotateState) {
        ...
        var _trWY = pos.y - (self._viewScrollY || 0);
        var curAngleT = Math.atan2(_trWY - trs.tube.y, pos.x - trs.tube.x);
      ...
      if (self._tubeLengthState) {
        ...
        var _tlWY = pos.y - (self._viewScrollY || 0);
        var delta = Math.hypot(pos.x - tls.startX, _tlWY - tls.startY);
        var sign = ((pos.x - tls.startX) * Math.cos(tls.tube.rotation) +
                    (_tlWY - tls.startY) * Math.sin(tls.tube.rotation)) > 0 ? 1 : -1;
      ...
      if (self._tubeDragging) {
        ...
        var _tWorldY = pos.y - (self._viewScrollY || 0);
      ...
      if (self._editorMode) {
        ...
        self._editorOnMove({x: pos.x, y: pos.y - (self._viewScrollY || 0)}); return;
      }
      ...
      var pos = getPos(e);
      var _wY = pos.y - (self._viewScrollY || 0);
      self.sling.pullX = pos.x; self.sling.pullY = _wY;
      var dx = self.sling.anchorX - pos.x, dy = self.sling.anchorY - _wY;
  ...
  _draw() {
    ...
    // Apply view scroll — shifts play area + editor panel together as one unit
...
// Replaces lines 3437-4103 in game.js
  _drawEditor() {
    ...
    var vSY   = 0;  // scroll handled by canvas translate in _draw(), not here
    var cY    = panelY;  // no scroll offset — canvas translate shifts everything
